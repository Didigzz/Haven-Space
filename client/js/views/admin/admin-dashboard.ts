...
import { getAuthHeadersOnly } from '../../shared/auth-headers.ts';
...
async function loadVerification() {
  ...
  try {
    const { data } = await adminApi('landlords?status=pending');
    if (!data.length) {
      list.innerHTML =
...
async function openLandlordDetail(id) {
  try {
    const { data } = await adminApi(`landlords?id=${id}`);
    const { data: history } = await adminApi(`landlords?history=${id}`);
...
async function loadProperties() {
  ...
  body.innerHTML = '<tr><td colspan="6" style="color:var(--admin-text-muted)">Loading…</td></tr>';
  try {
    const { data } = await adminApi(`properties?moderation=${encodeURIComponent(mod)}`);
    if (!data.length) {
      body.innerHTML =
...
/**
 * @param {object} currentUser from /api/auth/me
 */
