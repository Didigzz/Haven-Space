...
async function loadPropertyData(id) {
  try {
    const response = await fetch(`${CONFIG.API_BASE_URL}/api/landlord/properties?id=${id}`, {
...
async function loadDraftPropertyData(_profileId) {
  try {
    ...
    const userRes = await fetch(`${CONFIG.API_BASE_URL}/auth/me`, {
      headers: getAuthHeaders(),
      credentials: 'include',
    });
    if (!userRes.ok) {
...
function setupDragToReorder(grid) {
  ...
  function handleDrop(e) {
    ...
    if (dragSrcEl && dragSrcEl !== targetItem) {
      // Get the photo IDs
      const srcId = parseFloat(dragSrcEl.dataset.photoId);
      const targetId = parseFloat(targetItem.dataset.photoId);
...
async function handleFormSubmit(e) {
  ...
  try {
    ...
    if (isDraft) {
      ...
      response = await fetch(`${CONFIG.API_BASE_URL}/api/landlord/properties`, {
        method: 'POST',
        headers: getAuthHeaders(),
