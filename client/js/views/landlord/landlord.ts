...
import { getImageUrl } from '../../shared/image-utils.ts';
...
export function initLandlordDashboard(config = {}) {
  ...
  if (!isListingsPage && !isBoardersPage) {
    ...
    if (isDashboardPage) {
      import(`./landlord-applications.ts?v=${Date.now()}`).then(({ initLandlordApplications }) => {
...
async function loadDashboardData() {
  try {
    const response = await fetch(`${CONFIG.API_BASE_URL}/api/landlord/dashboard-stats`, {
      method: 'GET',
...
export async function sendPaymentReminder(paymentId, tenantName) {
  try {
    const response = await fetch(`${CONFIG.API_BASE_URL}/api/landlord/send-reminder`, {
      method: 'POST',
      headers: {
        ...getAuthHeaders(),
...
async function loadRecentActivities() {
  ...
  try {
    const response = await fetch(`${CONFIG.API_BASE_URL}/api/landlord/activity`, {
      method: 'GET',
      headers: getAuthHeaders(),
      credentials: 'include',
    });
...
async function loadPaymentOverview() {
  ...
  try {
    const response = await fetch(`${CONFIG.API_BASE_URL}/api/landlord/payment-overview`, {
      method: 'GET',
      headers: getAuthHeaders(),
      credentials: 'include',
    });
