...
async function loadDocuments() {
  ...
  try {
    const response = await fetch('/server/api/routes/landlord/documents', {
      method: 'GET',
...
async function toggleAutoSend(documentId, isActive) {
  try {
    const response = await fetch('/server/api/routes/landlord/documents/auto-send', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
async function deleteDocument(documentId) {
  try {
    const response = await fetch(`/server/api/routes/landlord/documents/${documentId}`, {
      method: 'DELETE',
    });
