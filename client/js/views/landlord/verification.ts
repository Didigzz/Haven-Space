...
async function loadVerificationStatus() {
  try {
    const response = await fetch(`${CONFIG.API_BASE_URL}/auth/verification-status`, {
      method: 'GET',
      credentials: 'include',
      headers: {
...
async function uploadDocument(formData) {
  try {
    const response = await fetch(`${CONFIG.API_BASE_URL}/landlord/upload-verification-document`, {
      method: 'POST',
      credentials: 'include',
      headers: {
        Authorization: `Bearer ${localStorage.getItem('token')}`,
