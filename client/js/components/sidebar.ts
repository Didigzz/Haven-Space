...
import { getAvatarUrl, getUserInitials } from '../shared/profile-utils.ts';
import { getHomePath, getLoginPath, isSourceViewsRuntime } from '../shared/paths.ts';
  ...
  admin: [
    {
      ...
      items: [
        { label: 'Overview', href: '../admin/index.html#overview', icon: 'home' },
        {
          ...
          href: '../admin/index.html#verification',
          icon: 'shieldCheck',
        },
...
function updateUserInfo(user) {
  ...
  if (avatar && avatarImg && avatarInitials) {
    ...
    if (user.avatar_file_id || user.avatar_url) {
      // Fetch avatar URL from profile-utils.ts (handles all formats including Google)
  ...
  if (logoutBtn) {
    logoutBtn.addEventListener('click', async e => {
      ...
      try {
        ...
        const basePath = resolveBasePath();
        const { logout } = await import(`${basePath}/js/shared/auth-check.ts`);
