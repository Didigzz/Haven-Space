...
export async function reverseGeocode(lat, lng) {
  try {
    ...
    const response = await fetch(`${CONFIG.API_BASE_URL}/geocode/search?lat=${lat}&lng=${lng}`);
...
export async function searchAddress(query, options = {}) {
  ...
  try {
    ...
    const response = await fetch(
      `${CONFIG.API_BASE_URL}/geocode/search?q=${encodeURIComponent(query)}&limit=${limit}`
    );
