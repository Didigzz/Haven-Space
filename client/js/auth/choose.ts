...
import { getBasePath } from '../shared/routing.ts';
...
async function checkPendingRegistration(token) {
  try {
    const response = await fetch(
      `${CONFIG.API_BASE_URL}/api/auth/google/check-pending-registration?token=${encodeURIComponent(
        token
      )}`,
      {
        method: 'GET',
  ...
  continueBtn.addEventListener('click', async () => {
    ...
    if (isGoogleOAuth) {
      ...
      try {
        ...
        const response = await fetch(
          `${CONFIG.API_BASE_URL}/api/auth/google/complete-registration`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
