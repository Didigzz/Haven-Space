...
try {
    ...
    $stmt = $pdo->prepare('SELECT id, first_name, last_name, email, role, is_verified FROM users WHERE google_id = ?');
    $stmt->execute([$googleId]);
    $user = $stmt->fetch();
    
    // If no user found by Google ID, check by email
    if (!$user) {
        $stmt = $pdo->prepare('SELECT id, first_name, last_name, email, role, password_hash, is_verified FROM users WHERE email = ?');
        $stmt->execute([$email]);
        $user = $stmt->fetch();
