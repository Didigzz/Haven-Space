...
ADD COLUMN IF NOT EXISTS gender_preference ENUM('male', 'female', 'any') NOT NULL DEFAULT 'any'
AFTER house_rules;

-- Add property_rules column (stores rules as TEXT)
ALTER TABLE properties
ADD COLUMN IF NOT EXISTS property_rules TEXT NULL
AFTER gender_preference;

-- Add index for gender_preference for filtering
ALTER TABLE properties
ADD INDEX IF NOT EXISTS idx_gender_preference (gender_preference);
