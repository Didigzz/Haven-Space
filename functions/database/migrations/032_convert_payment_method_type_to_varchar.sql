...
SET @dbname = DATABASE();
SET @tablename = 'payment_methods_landlord';
...
SET @sql = IF(@columnexists > 0,
    'ALTER TABLE payment_methods_landlord DROP FOREIGN KEY payment_methods_ibfk_2',
...
SET @sql = IF(@columnexists > 0,
    'ALTER TABLE payment_methods_landlord DROP COLUMN payment_method_type_id',
...
SET @sql = IF(@methodtypeexists = 0,
    'ALTER TABLE payment_methods_landlord ADD COLUMN method_type VARCHAR(100) NOT NULL AFTER landlord_id',
...
SET @sql = IF(@indexexists = 0,
    'ALTER TABLE payment_methods_landlord ADD INDEX idx_method_type (method_type)',
